...
public class apollo_main extends ListenerAdapter {
	...
	private helppage h = new helppage();
	//public List<Channel> chlist= new ArrayList<Channel>();
	private static String Token = "hier muss der Token rein";
	...
	public void onMessageReceived(MessageReceivedEvent event) {
		 ...
		 */}else if(msg.getContentRaw().substring(0,5).equals("!plan")) {
				...
				//	event.getChannel().sendMessage("Anfrage bekommen").queue();
					...
					//+2 da sonst / und Parameter fï¿½r Zeit mitgenommen wird
					...
					try {
						...
						event.getChannel().sendMessage("Fehler bei Zeitangabe").queue();;
					}
			...
			try {
				...
				int i=0;
					for(;i<m.size();i++) {
						...
						gegen +="<@"+m.get(i).getId()+">"+ " "; //<> werden bei laenge niccht mitgezÃ¤hlt
